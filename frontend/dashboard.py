...
REDIRECT_URI = 'http://localhost:8501' # Your streamlit redirect URI
...
# --- API Functions (for our backend) ---
def get_jds():
    try:
        response = requests.get(f"{API_BASE_URL}/jobs/")
        if response.status_code == 200:
            return response.json()
        st.error(f"Failed to fetch JDs: {response.text}")
...
with st.sidebar:
    ...
    selected_jd_title = st.selectbox("Select a Job Description", options=jd_options.keys(), index=None, placeholder="Choose a JD to analyze")
...
if not st.session_state.selected_jd:
    st.info("👋 Welcome to TalentLens! Please select a Job Description from the sidebar to begin.")
else:
    ...
    st.subheader("📤 Select Resumes for Analysis")
    ...
    with tab2:
        if not drive_service:
            ...
            if len(st.session_state.folder_path) > 1 and st.button("⬆️ Go to Parent Folder"):
            ...
            if st.session_state.drive_files:
                selected_drive_files = st.multiselect(
                    "Select resumes to analyze from the chosen folder:",
    ...
    if st.button("Analyze Resumes", use_container_width=True):
        ...
        if not api_key: st.error("Please enter your Gemini API Key in the sidebar.")
            ...
            progress_bar.empty()
            st.success("All resumes sent for analysis. Results will appear below. Click 'Refresh Results' to update.")
